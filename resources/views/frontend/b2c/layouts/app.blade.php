...
<head>
    ...
    <title>@yield('title', 'Customer Portal - V4 Kitchen Partner')</title>
        ...
        <div class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 w-full">
            <div class="flex justify-between items-center h-20">
                ...
                <div class="flex items-center gap-10">
                    <a href="{{ route('b2c.dashboard') }}" class="flex items-center group">
                    ...
                    <div class="hidden lg:flex items-center gap-1">
                        <a href="{{ route('b2c.dashboard') }}" class="nav-link {{ request()->routeIs('b2c.dashboard') ? 'active' : '' }}">
                        ...
                        <div class="relative has-dropdown">
                            <button class="nav-link {{ request()->routeIs('b2c.products') ? 'active' : '' }}">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="dropdown-menu">
                                ...
                                <div class="relative has-dropdown">
                                    <a href="{{ route('b2c.products', ['category' => $category->id]) }}" class="dropdown-item">
                                        {{ $category->name }}
                                    ...
                                    <div class="dropdown-menu dropdown-submenu">
                                        ...
                                        <a href="{{ route('b2c.products', ['category' => $category->id, 'subcategory' => $sub->id]) }}" class="dropdown-item font-medium">{{ $sub->name }}</a>
                                        @endforeach
                                    </div>
                                    @endif
                                </div>
                        ...
                        <a href="{{ route('b2c.orders') }}" class="nav-link {{ request()->routeIs('b2c.orders*') ? 'active' : '' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <a href="{{ route('b2c.profile') }}" class="nav-link {{ request()->routeIs('b2c.profile') ? 'active' : '' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <a href="{{ route('cart.index') }}" class="relative p-2 text-slate-600 hover:text-red-600 transition-colors">
                        ...
                        @if($cartCount > 0)
                        <span class="absolute top-0 right-0 w-4 h-4 bg-red-600 text-white text-[10px] font-black rounded-full flex items-center justify-center border-2 border-white">{{ $cartCount }}</span>
                        @endif
                    </a>
                    ...
                    <div class="relative has-dropdown">
                        ...
                        <div class="dropdown-menu left-auto right-0">
                            <a href="{{ route('b2c.profile') }}" class="dropdown-item">Profile Settings</a>
                    ...
                    <a href="{{ route('register') }}" class="nav-link" style="background: linear-gradient(135deg, #E31E24 0%, #B91C1C 100%); color: white; box-shadow: 0 12px 24px -6px rgba(227,30,36,0.35);">Register</a>
                    @endauth
    ...
    <div id="mobileSidebar" class="mobile-sidebar" onclick="toggleMobileMenu()">
        <div class="mobile-sidebar-content p-6" onclick="event.stopPropagation()">
            ...
            <div class="space-y-2" style="max-height: 80vh; overflow-y: auto;">
                <a href="{{ route('b2c.dashboard') }}" class="dropdown-item font-bold py-4">Dashboard</a>
                ...
                <div x-data="{ expanded: false }" class="w-full">
                    ...
                    <div x-show="expanded" style="display: none;" class="pl-4 pr-2 space-y-1 pb-2">
                        <a href="{{ route('b2c.products') }}" class="dropdown-item py-2 text-sm font-bold text-red-600 italic">View All Products</a>
                        @foreach($navCategories as $category)
                            <div x-data="{ subExpanded: false }" class="w-full">
                                ...
                                <div x-show="subExpanded" style="display: none;" class="pl-4 space-y-1 bg-slate-50 rounded-lg py-1 mt-1">
                                    <a href="{{ route('b2c.products', ['category' => $category->id]) }}" class="dropdown-item py-1.5 text-xs font-semibold italic text-slate-600">All {{ $category->name }}</a>
                                    @foreach($category->subcategories as $sub)
                                        <a href="{{ route('b2c.products', ['category' => $category->id, 'subcategory' => $sub->id]) }}" class="dropdown-item py-1.5 text-xs text-slate-500">{{ $sub->name }}</a>
                                    @endforeach
                                </div>
                                @endif
                            </div>
                ...
                <a href="{{ route('b2c.orders') }}" class="dropdown-item font-bold py-4">My Orders</a>
                <a href="{{ route('b2c.profile') }}" class="dropdown-item font-bold py-4">Profile</a>
                @auth
                <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form-mobile').submit();" class="dropdown-item font-bold py-4 text-red-600">Logout</a>
