...
<div class="max-w-4xl mx-auto">
    <nav class="flex mb-6 text-sm text-slate-500" aria-label="Breadcrumb">
        <ol class="flex items-center space-x-2">
            ...
            <li class="font-medium text-slate-900 italic">{{ $category->name }}</li>
            <li><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg></li>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
            <div>
                <h3 class="text-xl font-bold text-slate-900">Edit Category Details</h3>
                <p class="text-sm text-slate-500 mt-1">Modify the category information and specific dealer discounts.</p>
            </div>
            <span class="px-3 py-1 rounded-full text-xs font-bold {{ $category->is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600' }}">
                {{ $category->is_active ? 'Currently Active' : 'Currently Inactive' }}
            </span>
        </div>

        <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-y-6">
                ...
                <div class="space-y-2">
                    ...
                    @if($category->image)
                        <div class="mb-4">
                            <img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}" class="w-32 h-32 object-cover rounded-xl border border-slate-200 shadow-sm">
                    ...
                    >
                    <p class="text-xs text-slate-400 italic">Upload a new image to replace the current one.</p>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <input 
                            ...
                            id="b2b_discount" 
                            name="b2b_discount" 
                            value="{{ old('b2b_discount', $category->b2b_discount) }}" 
                    ...
                    <p class="text-xs text-slate-400 italic">Discount percentage applied to B2B customers for this category.</p>
                </div>
                ...
                <div class="pt-6 border-t border-slate-100">
                    ...
                    <div class="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-white rounded-xl border border-slate-200 shadow-sm transition-all hover:border-indigo-200">
                            ...
                            <div class="relative sm:w-32">
                                <input 
                                    ...
                                    max="100"
                                    step="0.01"
                                    value="{{ old('dealer_discounts.' . $dealer->id, $category->dealerDiscounts->where('customer_id', $dealer->id)->first()?->discount_percentage) }}"
                                >
                ...
                <div class="flex items-center space-x-3 pt-4">
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <span class="ml-3 text-sm font-semibold text-slate-700">Category Active Status</span>
                    </label>
                </div>
            </div>
