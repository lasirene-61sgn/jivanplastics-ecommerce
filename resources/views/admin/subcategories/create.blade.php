...
@section('title', 'Create Subcategory - Admin Panel')

@section('header', 'Create New Subcategory')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="p-8 border-b border-slate-100 bg-slate-50/50">
            <h3 class="text-xl font-bold text-slate-900">Subcategory Information</h3>
            <p class="text-sm text-slate-500 mt-1">Select a parent category and define the subcategory details.</p>
        </div>

        <form action="{{ route('admin.subcategories.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            @csrf
            
            <div class="grid grid-cols-1 gap-y-6">
                <div class="space-y-2">
                    <label for="category_id" class="text-sm font-semibold text-slate-700 flex items-center">
                        Parent Category <span class="text-red-500 ml-1">*</span>
                    </label>
                    <select name="category_id" id="category_id" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all outline-none bg-white" required>
                ...
                <div class="space-y-2">
                    <label for="name" class="text-sm font-semibold text-slate-700 flex items-center">
                        Subcategory Name <span class="text-red-500 ml-1">*</span>
                    </label>
                    <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="e.g. Running Shoes" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all outline-none" required>
                <div class="space-y-2">
                    ...
                    >
                    <p class="text-xs text-slate-400 italic">Images will be displayed in the B2B marketplace subcategory grid.</p>
                </div>

                <div class="space-y-2">
                    <label for="description" class="text-sm font-semibold text-slate-700">Description</label>
                    <textarea id="description" name="description" placeholder="Optional description..." class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all outline-none min-h-[100px]">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="flex items-center space-x-3 pt-4">
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <span class="ml-3 text-sm font-semibold text-slate-700">Active Status</span>
                    </label>
                </div>
            </div>
