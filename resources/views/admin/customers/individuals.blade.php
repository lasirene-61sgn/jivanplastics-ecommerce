...
@section('title', 'Individual Customers - Admin Panel')
...
<div class="space-y-6">
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-900">Individual Customers</h2>
            <p class="text-sm text-slate-500 font-medium italic">Direct-to-consumer (B2C) registered users.</p>
        </div>
        <div class="flex flex-wrap items-center gap-3">
            <div class="flex bg-slate-100 p-1 rounded-xl mr-2 border border-slate-200">
                ...
                <a href="{{ route('admin.customers.individuals') }}" class="px-4 py-2 text-xs font-bold rounded-lg bg-white text-indigo-600 shadow-sm">Individuals</a>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                <td class="px-6 py-4">
                                    <a href="{{ route('admin.customers.show', $customer) }}" class="flex items-center">
                                        <div class="h-10 w-10 rounded-full bg-sky-50 text-sky-600 flex items-center justify-center font-bold border border-sky-100 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-sm">
                                            {{ substr($customer->name, 0, 1) }}
                                        </div>
                                        <div class="ml-4 truncate max-w-[200px]">
                                            <div class="text-sm font-bold text-slate-900 group-hover:text-indigo-600 transition-colors truncate">{{ $customer->name }}</div>
                                            <div class="text-[10px] text-slate-400 font-medium">{{ $customer->city ?? 'Location not set' }}</div>
                                        </div>
                                    </a>
                                </td>
                                <td class="px-6 py-4 text-sm font-medium text-slate-600">{{ $customer->email }}</td>
                                <td class="px-6 py-4 text-xs font-mono text-slate-400 tracking-tighter">{{ $customer->phone ?? '—' }}</td>
                                <td class="px-6 py-4 text-center">
                                    @if($customer->is_active)
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase bg-green-100 text-green-700 tracking-wider">Active</span>
                                    @else
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase bg-red-50 text-red-500 tracking-wider">Disabled</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end space-x-2">
                                        ...
                                        <form action="{{ route('admin.customers.destroy', $customer) }}" method="POST" class="inline" onsubmit="return confirm('Archive this customer profile?')">
                                            @csrf @method('DELETE')
        ...
        @else
            <div class="p-20 text-center">
                <p class="text-slate-500 italic">No retail customers found.</p>
            </div>
        @endif
    </div>
</div>
